...
class _InboxState extends State<Inbox> {
  String id="", email="", fullname="", password="", token="", userRef="", reply="", recipRef="", unReadMsg="";
  List conversations=[];
  getPref() async {
  ...
  fetchConversations() async{
    final response = 
    await http.post(Uri.parse("https://k-dev.org/postbird/"), headers:{"Authorization" : token}, body:{
        "user": userRef
      }    
    );
    ...
    if(reqData['done'] == true){  
      ...
      for(var i = 0; i < dat.length; i++){      
        ...
        if(row['success'] == true){
          var c = [];
          var conv = {'name':row['data'][0]['name'], 'pic':row['data'][0]['profilepic'], 'msg': lastMsg, 'date':date, 'recipRef':recipRef, 'colorB': color, 'unr': unr };
          c.add(conv);
          print(conv['color']);
                                                ...
                                                Padding(
                                                  ...
                                                  child: CircleAvatar(
                                                    ...
                                                    backgroundImage: NetworkImage(
                                                      'https://api.postbird.com.ng/public/img/profile/'+conversations[item.key]['pic'],
                                                    ),
                                                  ),
                                                ),
                                                SizedBox(
